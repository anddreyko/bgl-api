...
This document defines the canonical domain model for BoardGameLog: bounded contexts, aggregates, entities, value objects,
and their invariants. It serves as the source of truth for domain modeling decisions.

**Key principles:**
- Inner layers never depend on outer layers (Clean Architecture)
- Bounded contexts communicate via IDs, not entity references
- Each context has its own representation of shared concepts (e.g., User is referenced by ID in Plays)
...
- **Auth** -- authentication/authorization infrastructure. Contracts in `Core/Auth/`, implementations in `Infrastructure/Auth/`.
...
| image            | string   | no       | URL                     |
| family           | string   | no       | Informational only, community-maintained |

**Invariants:**
- BGG ID is unique in the system
- Name is required
- Games are imported from BGG only, users do not create Game entities manually
- When user searches and is connected to BGG, their games are prioritized in results
