...
final class PlainTokenizerCest
{
    ...
    {
        $payload = TokenPayload::fromArray(['user_id' => '456', 'scope' => 'read']);
        $token = $this->generator->generate($payload, 3600);
        $result = $this->generator->verify($token);

        $i->assertTrue($result->has('user_id'));
        $i->assertTrue($result->has('scope'));
        $i->assertEquals('456', $result->getString('user_id'));
        $i->assertEquals('read', $result->getString('scope'));
    }
    ...
    public function testExpiredTokenThrowsException(UnitTester $i): void
    {
        ...
        $token = $pastGenerator->generate(TokenPayload::fromArray(['user_id' => '456']), 60);
